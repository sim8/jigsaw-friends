...
import { getPieceFromKey, getPieceHeight, getPieceWidth } from '../utils/pieces';
...
export default function Piece({ pieceKey, pieceState, jigsawConfig }: Props) {
  ...
  const pieceWidth = getPieceWidth(jigsawConfig.jigsawWidth, jigsawConfig.columns)
  const pieceHeight = getPieceHeight(jigsawConfig.jigsawHeight, jigsawConfig.rows)

  const { top, left, rotation} = pieceState

  return <div key={pieceKey} style={{
